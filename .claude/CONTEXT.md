# downloader — контекст проекту

## Що це

Python-програма для завантаження відео/аудіо з YouTube та SoundCloud через `yt-dlp`. Має два інтерфейси: CLI (термінал) і веб (браузер).

## Структура

```
downloader/
├── downloader.py       — CLI-програма (весь код завантаження)
├── web.py              — Flask веб-сервер (імпортує з downloader.py)
├── templates/
│   └── index.html      — HTML-сторінка веб-інтерфейсу
├── setup.sh            — встановлення залежностей на новому Mac (одноразово)
├── README.md           — інструкція для GitHub
├── .venv/              — віртуальне середовище (не в git)
├── .vscode/
│   └── settings.json   — інтерпретер .venv для VSCode
├── .gitignore          — ігнорує .venv, __pycache__, .DS_Store
└── .claude/
    └── CONTEXT.md      — цей файл, контекст для Claude
```

## Залежності

- `yt-dlp` — зовнішній CLI-інструмент. Встановлюється через `setup.sh`
- `flask` — для веб-інтерфейсу. Встановлюється через `setup.sh` у `.venv`

## Як запустити

```bash
python3 downloader.py   # CLI
python3 web.py          # веб → http://127.0.0.1:8080
```

## Архітектура downloader.py

- `find_ytdlp()` — шукає yt-dlp у стандартних шляхах
- `check_ytdlp()` — викликає `find_ytdlp`, виходить з помилкою якщо не знайдено
- `get_output_dir()` — повертає ~/Downloads або ~/Завантаження або поточну теку
- `run(ytdlp, args, output_dir)` — запускає yt-dlp з аргументами
- `show_menu(ytdlp, title, items, output_dir)` — інтерактивне меню вибору формату
- `main()` — головний цикл: YouTube / SoundCloud / змінити теку / вийти

## Архітектура web.py

- Імпортує `find_ytdlp`, `YT_MENU`, `SC_MENU`, `SEPARATOR` з `downloader.py`
- `GET /` — повертає HTML-сторінку з формою
- `POST /download` — отримує URL і формат, запускає yt-dlp у тимчасовій теці, повертає файл браузеру і видаляє тимчасову теку
- Тимчасові файли зберігаються у `/tmp/ytdlp-web/<uuid>/`
- Порт: 8080 (5000 зайнятий AirPlay на macOS)
- Помилки повертаються як plain text (не HTML) з кодом 500

## Веб-інтерфейс (index.html)

- Форма відправляється через `fetch` (не звичайний submit) — JS знає коли завантаження завершилось
- Кнопка під час завантаження: блокується, текст змінюється на "Завантаження" з анімованими крапками (CSS `@keyframes`)
- Після завершення (або помилки) кнопка повертається до "Завантажити"
- Назва файлу витягується з `Content-Disposition` заголовка (підтримує `filename*=UTF-8''` і звичайний `filename=`)

## Меню YouTube

- Відео — 1080p (H.264, сумісний з QuickTime)
- Відео — 720p (H.264, сумісний з QuickTime)
- Відео — 360p (H.264, сумісний з QuickTime)
- Відео — best (без обмежень — може бути 4K/VP9/AV1, потрібен VLC або IINA)
- Відео — worst (найнижча доступна якість, зазвичай 144p)
- Аудіо — AAC
- Аудіо — MP3 (bestaudio + --audio-quality 0, тобто максимальна якість)

## Меню SoundCloud

- AAC (оригінальний формат для Apple) — завантажує як є, без конвертації
- MP3 (найкраща якість для всіх пристроїв)
- FLAC (максимальна якість, але великі файли)

## Мережа (для розуміння)

- `127.0.0.1` — адреса "цього комп'ютера" (localhost), доступна тільки з самого Mac
- `0.0.0.0` — означає "слухати на всіх інтерфейсах", тобто сервер приймає з'єднання з будь-якого пристрою в мережі
- `8080` — порт, вказаний вручну в `web.py`. Можна змінити на будь-який інший
- Локальна IP — адреса Mac у домашній мережі. Змінюється при зміні мережі. Дізнатись: `ipconfig getifaddr en0`
- Сервер запущений з `host="0.0.0.0"` — доступний з телефону/інших пристроїв у тому ж Wi-Fi за адресою `http://<локальний-ip>:8080`
- **`.local` ім'я** — стабільніша альтернатива IP: `http://MacBookAirRoma.local:8080`. Не змінюється при зміні мережі, працює на будь-якому WiFi де є обидва пристрої. `web.py` виводить цю адресу при старті.
- Сайт доступний лише поки запущений `web.py` — зупинив процес, сайт недоступний
- Для доступу ззовні (друзі, інша мережа) — ngrok: `brew install ngrok` → `ngrok http 8080`. Дає тимчасове публічне посилання.

## Продакшн (на майбутнє)

Якщо розгортати на VPS (Hetzner):
- Прибрати `debug=True` з `app.run()`
- Запускати через gunicorn: `gunicorn -w 4 web:app`
- Поставити nginx як reverse proxy
- Порт, `.local`, ngrok — не потрібні, бо є постійна публічна IP або домен

## Важливо

- mp4 формати використовують `[vcodec^=avc1]` для H.264 — відкриваються в QuickTime
- "Найкраща якість відео" без обмежень по кодеку — для 4K потрібен IINA або VLC
- `SEPARATOR = None` — використовується для порожнього рядка між пунктами меню
- Тимчасові файли CLI (потоки до злиття, `.part`) йдуть у `/tmp/yt-dlp-tmp`
- Веб-інтерфейс: браузер чекає поки завантажиться файл (без реального прогрес-бару, є анімація кнопки)
